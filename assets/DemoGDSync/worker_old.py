...
3. **Updates its local model parameters** using gradient descent.
4. **Interacts with neighboring workers** (if applicable) to regularize updates.
5. **Simulates processing delays** based on an adjustable speed factor.
...
4. Upon sync, it updates `w` using:
   - Local gradient descent.
   - A smoothing term from neighboring worker values.
5. The process repeats until manually stopped.
